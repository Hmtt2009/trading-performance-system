...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const formatName = formData.get('formatName') as string | null;
    ...
    if (!mapping.symbol || !mapping.dateTime || !mapping.quantity || !mapping.price) {
      return NextResponse.json(
        { error: 'Column mapping must include symbol, dateTime, quantity, and price.' },
        { status: 400 }
      );
    }

    const headerRowIndex = headerRowIndexStr ? parseInt(headerRowIndexStr, 10) : 0;
    if (isNaN(headerRowIndex) || headerRowIndex < 0) {
      return NextResponse.json(
        { error: 'Invalid header row index' },
        { status: 400 }
      );
    }
    ...
    // Parse the CSV with the provided mapping
    const parseResult = parseWithMapping(csvContent, mapping, headerRowIndex, existingHashes);
    ...
    await supabase
      ...
      .update({
        status: uploadStatus,
        broker_format: formatName || 'manual',
    ...
    // Compute/update baseline and sessions (same logic as main upload route)
    ...
    if ((totalTradeCount ?? 0) > TRADE_LIMIT) {
      console.error(`User ${user.id} has ${totalTradeCount} trades, exceeding ${TRADE_LIMIT} limit.`);
    }
    ...
    console.error('Upload with mapping error:', error);
