...
function toScoreEntry(raw: RawSegment): ScoreEntry {
  return {
    ...
    profitFactor: raw.lossPnl !== 0 ? raw.winPnl / Math.abs(raw.lossPnl) : raw.winPnl > 0 ? 999.99 : 0,
  };
}
            ...
            {sortedEntries.map(([key, entry]) => {
              ...
              return (
                <tr key={key} className={`border-b border-border/30 hover:bg-surface transition-colors ${isSignificantlyNegative ? 'bg-red/5' : ''}`}>
                  ...
                  <td className="px-4 py-2 text-right text-muted font-mono">{entry.profitFactor >= 999.99 ? '--' : entry.profitFactor.toFixed(2)}</td>
                </tr>
              );
            })}
          </tbody>
